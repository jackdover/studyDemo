...
public class LockViewGroup extends RelativeLayout {
    ...
    private float mMarginRate = 0.75f;
...
//            mLockViews[i].setMode(LockView.Status.STATUS_NORMAL); //减少重绘
    ...
    public void dispatchDraw(Canvas canvas) {
        ...
        Log.e(TAG, "dispatchDraw, mPath=" + mPath);
    }
    ...
    private void doSaveCheckResult() {
        ...
        if (mChoose.size() < 4) {   //1.1 不合法
            // 绘制错误界面, 并且一定时间后消失 //TODO
            drawError();
            //少于4点的提示回调
            if (mLockListener != null) {
                mLockListener.onLess4Points();
            }
        } else {    //1.2 合法,去校验

            //1.2.1 如果是第一次 --- 还原默认界面 + 保存答案 + 执行保存回调
            if (mFirstAnswer == null) {
                ...
                if (checkAnswer()) { //.1 正确 --- 还原默认 + 执行成功回调
                    ...
                    // 执行次数提示回调
                    if (mLockListener != null) {
                        mLockListener.onTryTimes(this.mTryTimes);
                    }
                }
            }
        }
     ...
     * @param config
     */
    public void setmConfig(LockViewConfig config) {
        this.mConfig = config;
    }
