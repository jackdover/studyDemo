...
public class GestureLockViewGroup extends RelativeLayout {
     ...
     */
    private int mNoFingerOuterCircleColor = 0xFF0F8EE8;//0xFFE0DBDB;
    /**
     * GestureLockView手指触摸的状态下内圆和外圆的颜色
     */
    private int mFingerOnColor = 0xFF2177C7;//0xFF378FC9;
    /**
...
//        a.recycle();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mLockViews == null) {
            ...
            mPaint.setStrokeWidth(4);//mGestureLockViewWidth * 0.29f
...
//                        mFingerOnColor, mFingerUpColor);
                ...
                addView(mLockViews[i], lockerParams);
            }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                mPaint.setAlpha(50);
                LockView child = getChildIdByPos(x, y);
                if (child != null) {
                    int cId = child.getId();
                    if (!mChoose.contains(cId)) {
                        mChoose.add(cId);
                        child.setStatus(LockView.Status.STATUS_ON);
...
//                            //获取前一个
//                            GestureLockView preChild = (GestureLockView) findViewById(mChoose.get(mChoose.size() - 2));
//                            int dx = child.getLeft() - preChild.getLeft();
//                            int dy = child.getTop() - preChild.getTop();
//                            // 计算角度
//                            int angle = (int) Math.toDegrees(Math.atan2(dy, dx)) + 90;
//                            preChild.setArrowDegree(angle);
//                            invalidate();
                        }
            ...
            case MotionEvent.ACTION_UP:
                ...
                for (int i = 0; i + 1 < mChoose.size(); i++) {
                    ...
                    LockView startChild = (LockView) findViewById(childId);
     ...
     * @return
     */
    private LockView getChildIdByPos(int x, int y) {
        for (LockView lockView : mLockViews) {
            if (checkPositionInChild(lockView, x, y)) {
                return lockView;
            }
        }
